...
$(document).ready( () => {
    ...
    $("#corporate, #individual").change( () => { //selector for all radio buttons, using a # allows to select the id instead of the type of element
        const radioButton = $("#corporate:checked, #individual:checked").val(); //this one selects the radio button that is already checked
    ...
    $("#member_form").submit( event => {
        ...
        const passCap = /[\.]*[A-Z][\,]*/;
        if ( password.length < 6 || passCap.test(password)) {
            $("#password").next().text(">=6 characters, >= cap.");
            isValid = false;
        } else {
            $("#password").next().text("");
        }
        ...
        const phonePattern = /^\d{3}-\d{3}-\d{4}$|^\(d{3})-\d{3}-\d{4}$/; //The ^ and the $ are there to format the phone number expression properly
         //this form to include the () for the first 3 digits
        const phone = $("#phone").val().trim();
        if (phone == "") { 
            $("#phone").next().text("This field is required.");
            isValid = false; 
        } else if ( !phonePattern.test(phone) ) {
            $("#phone").next().text("Use 999-999-9999 format or Use (999)-999-9999 format.");
            isValid = false;
        } else {
            $("#phone").next().text("");
        }
